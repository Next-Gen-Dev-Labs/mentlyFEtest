			...
			{mentors.map(({ imageUrl, name, role, id }) => (
				<Fragment key={id}>
					<div className="flex items-center justify-between gap-2 py-2 text-xs last-of-type:mb-8">
						...
						<button
							type="button"
							className="rounded-full bg-[#6f01d0] px-3 py-1 text-white outline-1 -outline-offset-1 outline-transparent transition hover:bg-white hover:text-[#6f01d0] hover:outline-current focus-visible:bg-white focus-visible:text-[#6f01d0] focus-visible:outline-current active:scale-95"
						>
							Message
						</button>
					</div>
