...
const WidgetBase = memo((props: PropsWithChildren<WidgetBase>) => {
	...
	return (
		...
		>
			<header className="sticky flex items-center gap-3 bg-gradient-to-b from-white to-transparent pb-5">
				...
				<h2 className="mr-auto truncate font-bold">{props.title}</h2>
				{props.extraHeaderActions}
				{props.seeAllAction && (
					<button
						type="button"
						className="shrink-0 rounded-xs px-1.5 text-xs font-semibold text-[#6f01d0] outline outline-transparent transition-colors hover:bg-current/5 hover:outline-current/25 focus-visible:bg-current/5 focus-visible:outline-current/25"
				...
				{props.moreOptions && props.moreOptions.length > 0 && (
					<>
						...
						<div
							popover=""
							className="inset-auto top-[anchor(bottom)] right-[anchor(right)] m-0 mr-[anchor-size(width)] h-auto w-28 flex-col items-stretch [justify-self:anchor-center] rounded-sm bg-white py-0.75 text-left text-sm shadow open:flex"
