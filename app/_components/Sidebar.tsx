import Image from "next/image";
import Link from "next/link";
import sidebarLinks from "../_constants/sidebar";
import ToggleButton from "./ToggleButton";

const Sidebar = function () {
  return (
    <aside className="bg-primary-400 p-4">
      <div className="grid place-items-center gap-[40px]">
        <Image src={"/images/logo.png"} alt="mently" width={47} height={32} />
        <Image
          src={"/icons/logo-icon.svg"}
          alt="mently"
          width={32}
          height={32}
        />
      </div>

      <ul className="mt-16 grid place-items-center gap-2 ">
        {sidebarLinks.map((sidebar) => (
          <li className="w-20 h-14  " key={sidebar.id}>
            <Link
              href={"/"}
              className={`w-full h-full grid place-items-center ${
                sidebar.id === 2 && "bg-white rounded-lg"
              }`}
            >
              {/* {sidebar.id} */}
              <Image
                src={sidebar.path}
                width={20}
                height={20}
                alt="dashboard"
              />
            </Link>
          </li>
        ))}
      </ul>

      <div>
        <Image
          src={"/icons/user-tag.svg"}
          width={20}
          height={20}
          alt="user icon"
        />
        <div className="text-secondary-400">Visit Mently Help Desk Here</div>
      </div>
      <ToggleButton />
    </aside>
  );
};

export default Sidebar;

// rgba(255, 255, 255, 0.12)

// background: var(--Action-Fill-Neutral-Focus-Ghost-Inverted, #FFFFFF1F);
