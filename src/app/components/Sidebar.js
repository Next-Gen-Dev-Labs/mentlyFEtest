...
const Sidebar = () => {
    ...
    return (
        <div className={`absolute z-30 bg-[#340260] h-full transition-all duration-300 overflow-y-auto px-2 ${toggleSidebar ? 'w-[4.5%]' : 'w-[18.75%]'}`}>
            ...
            <div className={`flex flex-col items-center px-1 mt-7 ${toggleSidebar? 'gap-3':'gap-1'}`}>
                {sidebarItems.map((item, index) => (
                    <button
                        key={index}
                        onClick={() => handleChange(item.id)}
                        className={`flex items-center justify-start gap-10 p-2 rounded-md transition-all duration-300 w-full
                    ...
                    >
                        ...
                        />
                        <p className={`${toggleSidebar ? 'hidden' : 'block'} text-base`}>{item.label}</p>
                    </button>
                ))}
            </div>
