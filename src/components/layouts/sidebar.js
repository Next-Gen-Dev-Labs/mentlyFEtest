...
export default function SideBar() {
    ...
    const [isCollapsed, setIsCollapsed] = useState(false); // State to toggle collapse on larger screens
    ...
    return (
        <>
            {/* Menu Icon for Mobile and Tablet */}
                ...
                <div className="w-full flex -top-0.5  items-center justify-between lg:hidden fixed z-50  bg-[#FDFDFD] py-4 px-3" style={{boxShadow: '0px 2px 4px 0px #00000010'}}>
            ...
            {/* Sidebar */}
            <div className={`fixed lg:static top-0 left-0 h-full  bg-[#340260] z-40 transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 px-4 md:space-y-[89px] pt-20 md:pt-9 pb-5 md:pb-12`}>
                ...
                <div>
                    <ul className="text-white space-y-1 md:space-y-2 mb-6 md:mb-12">
                        {menus.map((menu) => (
                            ...
                            >
                                ...
                                {!isCollapsed && <p className="text-sm font-semibold lg:hidden xl:block">{menu.title}</p>}
                            </li>
                        ))}
                    </ul>
                    <div className={`space-y-2.5 md:space-y-5 bg-[#FFFFFF1F] rounded-lg px-2 xl:px-4 py-2.5 mb-8 ${isCollapsed ? 'hidden' : ''}`}>
                    ...
                    {!isCollapsed && (
                        <div>
                            <p className="text-xs text-white font-bold">Switch to Classic Mode</p>
                        </div>
                    )}
                </div>
            </div>
